...
> **重要**: 下記 1 / 2 を **デプロイ前に必ず設定** してください。未設定で push すると次のエラーで失敗します:
> ```
> Error: At ValidateParameter, app-name : should not be empty.
> ```
> これはワークフローが `${{ vars.AZURE_FUNCTIONAPP_NAME }}` を参照しているのに、その値が空のまま `Azure/functions-action@v1` に渡されたことを意味します。

### 1. Repository variables（公開設定 OK・**必須**）

GitHub には変数のスコープが複数あります。本リポジトリでは **Repository variables** を使います（Environment variables ではありません）。

設定場所:
1. GitHub のリポジトリページを開く
2. `Settings` → `Secrets and variables` → `Actions`
3. **`Variables`** タブ（`Secrets` タブの隣）を選択
4. **`Repository variables`** セクション（ページ上部、`Environment variables` セクションではない方）の `New repository variable` をクリック
5. 以下 2 件を登録:

| 変数名 | 値の例 | 用途 |
|---|---|---|
| `AZURE_FUNCTIONAPP_NAME` | `func-flutter-build` | デプロイ先 Function App 名（**未設定だと上記エラー**） |
| `AZURE_RESOURCE_GROUP` | `rg-flutter-build` | Function App のリソースグループ名 |

> ※ `Environment variables` セクション（GitHub Environment に紐づく変数）に登録しても、本ワークフローは `environment:` を指定していないため**読まれません**。必ず **Repository variables** に入れてください。

### 2. Repository secrets（機密・**必須**）

設定場所: 上記と同じページの **`Secrets`** タブ → **`Repository secrets`** → `New repository secret`

| シークレット名 | 取得方法 |
|---|---|
| `AZURE_FUNCTIONAPP_PUBLISH_PROFILE` | Azure ポータル → Function App `func-flutter-build` → 上部メニューの「発行プロファイルの取得（Get publish profile）」で DL した `.PublishSettings` XML の **中身をそのまま貼付** |

### 3. Function App 側で Basic 認証を有効化（**必須**）

新規作成された Function App は **SCM / FTP の Basic 認証がデフォルトで OFF** になっており、この状態で Publish Profile デプロイを実行すると次のエラーで失敗します:
...
`Azure/functions-action@v1` は内部で Kudu API（`<funcname>.scm.azurewebsites.net`）を Basic 認証で叩くため、有効化が必要です。

設定手順:
1. Azure ポータル → Function App `func-flutter-build`
2. 左メニュー **Settings → Configuration**（設定 → 構成）
3. **General settings**（全般設定）タブ
4. **「SCM Basic Auth Publishing Credentials」** を **On**
5. **「FTP Basic Auth Publishing Credentials」** も **On** 推奨
6. **Save**
...
### 5. デプロイ確認
```bash
curl https://<AZURE_FUNCTIONAPP_NAME>.azurewebsites.net/api/aci-status
```
